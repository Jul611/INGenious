...
**Key files**:
- `PluginRegistryCliService` — all `git`/`gh`/`mvn` process-shelling: fork/branch resolution, PR submission, registry reads, Maven artifact/dependency resolution, and `saveAdoCredential()` (writes a user-pasted ADO PAT into their real `~/.m2/settings.xml`, preserving anything else already there). `GH_OWN_GIT_CREDENTIAL_ENV` hands gh's own git credential helper to the `gh pr create` process via a scoped `GIT_CONFIG_COUNT` env override (not a persisted git-config change) — works around `gh pr create` and plain `git`/GCM operations otherwise being two independent credential paths that can each prompt for sign-in separately.
...
**Next task — the CI pipelines**: `validate.yml`/`publish.yml` exist only as templates; they aren't wired into a running repo yet, though the concrete steps to do so against the workaround repo are known (copy both workflow files into `.github/workflows/` there, edit their `branches: [main]` trigger to match whatever branch is actually in use, vendor `ingenious-api` into `ci/vendor/`, add the 5 repo Variables + the write-scoped `ADO_PAT` secret). Open decision before treating that as the permanent design: keep `publish.yml`'s ADO deploy step on GitHub Actions with a manually-owned, write-scoped `ADO_PAT` secret (simple, single CI system) — or split the merge-time build+deploy out to an Azure Pipeline that reuses the team's already-working `MavenAuthenticate@0` + `$(System.AccessToken)` pattern from their existing INGenious-build pipeline (no secret to own/rotate at all, but splits CI across GitHub Actions for PR validation and Azure Pipelines for the deploy step). Neither is implemented yet.
