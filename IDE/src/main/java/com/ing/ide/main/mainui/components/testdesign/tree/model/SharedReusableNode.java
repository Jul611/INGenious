...
public class SharedReusableNode extends CommonNode {
    ...
    private void filterGroups() {
        for (Scenario scenario : project.getScenarios()) {
            for (TestCase reusable : scenario.getSharedReusables()) {
                String groupName = reusable.getSharedReusable().getGroup();
   ...
   public static List<SharedReusableNode> toList(Enumeration<TreeNode> children){
       return Collections.list(children).stream().map(tsNode -> (SharedReusableNode) tsNode).collect(Collectors.toList());
