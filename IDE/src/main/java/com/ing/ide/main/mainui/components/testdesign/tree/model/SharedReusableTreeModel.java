...
import com.ing.datalib.component.SharedReusable;
...
public class SharedReusableTreeModel extends ProjectTreeModel {
    ...
    public TestCaseNode addTestCase(ScenarioNode scNode, TestCase testCase) {
        ...
        SharedReusable reusable = tcNode.getTestCase().getSharedReusable();
        if (reusable == null) {
            reusable = new SharedReusable();
        }
        reusable.setGroup(scNode.getParent().toString());
        tcNode.getTestCase().setSharedReusable(reusable);
        return tcNode;
    }
    ...
    private void createAndSetAttribute(Element parentElement, TestCase testCase) {
        ...
        newElement.setAttribute("exeType", testCase.getSharedReusable().getExecutableType());
        parentElement.appendChild(newElement);
    }
